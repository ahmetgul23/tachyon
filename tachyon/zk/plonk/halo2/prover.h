...
class Prover : public ProverBase<PCS> {
 public:
  ...
  [[nodiscard]] bool CreateProof(
    ...
    CHECK_EQ(num_circuits, instance_3d_vec.size());
    for (const std::vector<std::vector<F>>& instances_vec : instance_3d_vec) {
    ...
    ExtendedEvals circuit_evals = argument.GenerateCircuitPolynomial(
    ...
    CHECK(CommitFinalHPoly(this, std::move(committed_result).TakeVanishing(),
                           proving_key.verifying_key(), circuit_evals,
